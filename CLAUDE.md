...
- `src/hooks.server.ts` — middleware that Ed25519-verifies every `/api/room/` request (timestamp
  window + signature) against `K1` before it reaches a handler.
- `src/routes/api/room/[k1]/…` — the dumb R2 blob store (list / get / conditional put). R2 binding
  is `BUCKET`.
